	...
	},
	{
		...
		method: function (message, commandData) {
			...
			mal.searchAnimes(animeName).then(res => {
				...
				var exactAnime = res.find(o => o.english.trim().toLowerCase() == animeName.trim().toLowerCase() || o.title.trim().toLowerCase() == animeName.trim().toLowerCase());
				if (animes.length == 0) {
	...
	},
	{
		...
		method: function (message, commandData) {
			...
			mal.searchMangas(mangaName).then(res => {
				...
				var exactManga = res.find(o => o.english.trim().toLowerCase() == mangaName.trim().toLowerCase() || o.title.trim().toLowerCase() == mangaName.trim().toLowerCase())
				console.log(exactManga);
				if (mangas.length == 0) {
					message.reply("Couldn't find anything...");

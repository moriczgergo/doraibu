...
module.exports = [
	{
		...
		method: function (message, command, client) {
	...
	},
	{
		...
		method: function (message, commandData) {
			...
			mal.searchAnimes(animeName).then(res => {
				...
				// Find exact animes
				var exactAnime;
				if (animes.length > 0) exactAnime = res.find(o => o.english.trim().toLowerCase() == animeName.trim().toLowerCase() || o.title.trim().toLowerCase() == animeName.trim().toLowerCase());
				if (animes.length == 0) {
	...
	},
	{
		...
		method: function (message, commandData) {
			...
			mal.searchMangas(mangaName).then(res => {
				...
				// Find exact mangas
				var exactManga;
				if (mangas.length > 0) exactManga = res.find(o => o.english.trim().toLowerCase() == mangaName.trim().toLowerCase() || o.title.trim().toLowerCase() == mangaName.trim().toLowerCase())
				if (mangas.length == 0) {
					message.reply("Couldn't find anything...");
				} else if (mangas.length == 1 || exactManga) { // If only found one manga or found an exact manga, display it in detail
					var x = exactManga || res[0];
					var selectedManga = {
						title: x.title,
						description: he.decode(x.english + "\n\n" + x.type + " | " + x.status + "\n\n" + x.synopsis).replace(new RegExp("\\[i\\]", "g"), "*").replace(new RegExp("\\[/i\\]", "g"), "*").replace(new RegExp("\\[b\\]", "g"), "**").replace(new RegExp("\\[/b\\]", "g"), "**"),
						color: 0x0000FF,
	...
	{
		name: prefix + "aijoubu",
		helpText: prefix + "aijoubu",
		regex: new RegExp(prefix + "aijoubu", "i"),
		description: "It's 🇴🇰.",
		method: function (message) {
